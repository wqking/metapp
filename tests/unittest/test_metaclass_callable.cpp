...
struct metapp::DeclareMetaType <C> : metapp::DeclareMetaTypeBase <C>
{
	static void setup()
	{
		tmcMetaRepo.registerBase<C, B, B2>();
	}
...
TEST_CASE("MetaClass, callable, struct A")
{
	...
	const auto & virtualMethod = metaClassA->getCallable("virtualMethod");
	REQUIRE(metapp::callableInvoke(virtualMethod, pa).get<int>() == 11);
}

TEST_CASE("MetaClass, callable, struct B")
{
	...
	REQUIRE(! methodB.asCallable().isEmpty());
...
TEST_CASE("MetaClass, callable, struct C")
{
	auto metaTypeC = metapp::getMetaType<C>();
	auto metaClassC = metaTypeC->getMetaClass();

	C c;
	const auto & methodB2 = metaClassC->getCallable("methodB2");
	REQUIRE(metapp::callableInvoke(methodB2, &c).get<int>() == 8);

}

TEST_CASE("MetaClass, callable, struct C, getCallableView")
{
	auto metaTypeC = metapp::getMetaType<C>();
	auto metaClassC = metaTypeC->getMetaClass();

	std::map<std::string, int> callableNameMap;
	auto callableView = metaClassC->getCallableView();
	for(const auto & item : callableView) {
		++callableNameMap[item.getName()];
	}

	REQUIRE(callableNameMap["methodA"] == 1);
	REQUIRE(callableNameMap["virtualMethod"] == 1);
	REQUIRE(callableNameMap["methodB"] == 1);
	REQUIRE(callableNameMap["methodC"] == 1);
	REQUIRE(callableNameMap["notReflected"] == 0);
}
