...
TEST_CASE("metatypes, int *, accessible")
{
	int n = 5;
	metapp::Variant v(&n);
	REQUIRE(v.getMetaType()->getMetaAccessible()->accessibleGet(v, nullptr).get<int>() == 5);

	v.getMetaType()->getMetaAccessible()->accessibleSet(v, nullptr, 38);
	REQUIRE(n == 38);
	REQUIRE(v.getMetaType()->getMetaAccessible()->accessibleGet(v, nullptr).get<int>() == 38);

	v.getMetaType()->getMetaAccessible()->accessibleSet(v, nullptr, 98.0);
	REQUIRE(n == 98);
	REQUIRE(v.getMetaType()->getMetaAccessible()->accessibleGet(v, nullptr).get<int>() == 98);
}
