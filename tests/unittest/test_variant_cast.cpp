...
TEST_CASE("Cast object")
{
	...
	REQUIRE(v.cast<MyClass>().value == 38);
}
...
TEST_CASE("Cast std::string to char *")
{
	...
	REQUIRE(strcmp(v.cast<const char *>(), s.c_str()) == 0);
	REQUIRE(strcmp(v.cast<char *>(), s.c_str()) == 0);
}
...
TEST_CASE("Cast std::wstring to wchar_t *")
{
	...
	REQUIRE(wcscmp(v.cast<const wchar_t *>(), ws.c_str()) == 0);
